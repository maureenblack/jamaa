...
async function loadTherapists(filters = {}) {
    try {
        const therapistList = document.getElementById('therapistList');
        if (!therapistList) return;

        therapistList.innerHTML = '<div class="text-center"><div class="spinner-border" role="status"></div></div>';

        const therapists = await window.jamaaAPI.therapists.getAll(filters);
        ...
        therapists.forEach(therapist => {
            ...
            card.innerHTML = `
                <div class="card h-100">
                    <img src="${therapist.user.profilePicture}" class="card-img-top" alt="${therapist.user.name}">
                    <div class="card-body">
                        <h5 class="card-title">${therapist.user.name}</h5>
                        <p class="card-text">
                            <i class="fas fa-star text-warning"></i> ${therapist.rating.toFixed(1)}<br>
                            <i class="fas fa-tags"></i> ${therapist.specialties.join(', ')}<br>
                            <i class="fas fa-language"></i> ${therapist.languages.join(', ')}<br>
                            <i class="fas fa-dollar-sign"></i> $${therapist.pricePerSession}/session
                        </p>
                        <button class="btn btn-primary-jamaa book-session" data-therapist-id="${therapist._id}">
                            Book Session
                        </button>
                    </div>
                </div>
        ...
        document.querySelectorAll('.book-session').forEach(button => {
            button.addEventListener('click', () => bookSession(button.dataset.therapistId));
        });
    } catch (error) {
        console.error('Error loading therapists:', error);
        if (therapistList) {
            therapistList.innerHTML = '<div class="alert alert-danger">Error loading therapists. Please try again later.</div>';
        }
    }
}
...
async function bookSession(therapistId) {
    ...
    try {
        const therapist = await window.jamaaAPI.therapists.getById(therapistId);
        // Show booking modal with therapist details
        $('#bookingModal').modal('show');
        // Populate therapist details in the modal
        document.getElementById('bookingTherapistName').textContent = therapist.user.name;
        // Additional booking logic...
    } catch (error) {
        console.error('Error fetching therapist details:', error);
        alert('Error loading therapist details. Please try again later.');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                await window.jamaaAPI.auth.login(email, password);
    ...
    document.getElementById('registrationForm')?.addEventListener('submit', async (e) => {
        ...
        try {
            await window.jamaaAPI.auth.register(formData);
